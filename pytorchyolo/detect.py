...
def detect_image(model, image, img_size=416, conf_thres=0.5, nms_thres=0.5, classes=None, target=None):
    ...
    if torch.cuda.is_available():
        input_img = image.to("cuda")

    # Get detections
    detections = model(input_img)
    # print(f"Original shape: {detections.shape}")
    detections = non_max_suppression(detections, conf_thres, nms_thres, classes=classes, target=target)
