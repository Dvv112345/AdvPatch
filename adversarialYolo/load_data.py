...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], by_rectangle=False, do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False, enable_empty_patch=False, enable_no_random=False, enable_blurred=True):
        ...
        def resize_rotate(adv_batch, by_rectangle=False):
            ...
            cls_mask = cls_mask.unsqueeze(-1)  # torch.Size([8, 14, 3, 300, 1])
            cls_mask = cls_mask.expand(-1, -1, -1, -1, adv_batch.size(4))  # torch.Size([8, 14, 3, 300, 300])
            msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(1) - cls_mask.cuda()  # torch.Size([8, 14, 3, 300, 300])

            # Pad patch and mask to image dimensions
            # Determine size of padding
            pad_w = (img_size - msk_batch.size(-1)) / 2  # (416-300) / 2 = 58
            pad_h = (img_size - msk_batch.size(-2)) / 2
            # print("pad : "+str(pad))
            mypad = nn.ConstantPad2d((int(pad_w), int(pad_w), int(pad_h), int(pad_h)), 0)
            # print("adv_batch size : "+str(adv_batch.size()))
            adv_batch = mypad(adv_batch)  # adv_batch size : torch.Size([8, 14, 3, 416, 416])
            msk_batch = mypad(msk_batch)  # adv_batch size : torch.Size([8, 14, 3, 416, 416])
            # print("adv_batch size : "+str(adv_batch.size()))
