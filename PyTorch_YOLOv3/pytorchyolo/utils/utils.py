...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, target=None):
    ...
    t = time.time()
    output = [torch.zeros((1, 0, 6), device="cuda")] * prediction.shape[0]
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            i, j = (confidence > conf_thres).nonzero(as_tuple=False).T

            # x = torch.cat((box[i], confidence[i, j, None], j[:, None].float()), 1)
            if target is None:
                x = torch.cat((box[i], confidence[i, j, None], j[:, None].float()), 1)
            else:
                x = torch.cat((box[i], confidence[i, j, None], j[:, None].float(), confidence[i, j, None] - confidence[i,target, None]), 1)
